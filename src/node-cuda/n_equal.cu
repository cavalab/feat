...
namespace FT{
    ...
    __global__ void Equal(bool * x1, bool * x2, bool * out, size_t N)
    {                    
    ...
    {
        ArrayXb x2 = stack_f.back(); stack_f.pop_back();
        ArrayXb x1 = stack_f.back(); stack_f.pop_back();
        // evaluate on the GPU
        ...
        Equal<<< 32*numSMs, 128 >>>(dev_x1, dev_x2, dev_res, N);
