...
namespace FT{
    ...
    __global__ void Sqrt( float * x, size_t idx, size_t N)
    {                    
        for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < N; i += blockDim.x * gridDim.x)
        {
            x[(idx-1)*N+i] = sqrt(abs(x[(idx-1)*N+i]));
        }
        return;
    }
    void GPU_Sqrt( float * x, size_t idx, size_t N)
    {
        GPU_Sqrt<<< DIM_GRID, DIM_BLOCK, omp_get_thread_num() >>>( float * x, size_t idx, size_t N);
    }
    ...
    /*     Sqrt<<< 32*numSMs, 128 >>>(dev_x1, dev_res, N); */
