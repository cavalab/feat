...
namespace FT{
    ...
    __global__ void Multiply( float * x, size_t idx, size_t N)
    {                    
    ...
    {
        GPU_Multiply<<< DIM_GRID, DIM_BLOCK, omp_get_thread_num() >>>( float * x, size_t idx, size_t N);
    }
    /// Evaluates the node and updates the stack states. 
    /* void NodeMultiply::evaluate(const MatrixXd& X, const VectorXd& y, vector<ArrayXd>& stack_f, */ 
