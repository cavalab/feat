...
namespace FT{
   	namespace Pop{
   	    namespace Op{	
            ...
            __global__ void Exponent( float * x, size_t idx, size_t N, float W0, float W1)
            {                    
                for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < N; i += blockDim.x * gridDim.x)
                {
                    x[(idx-2)*N+i] = limited(pow(W0*x[(idx-1)*N+i] , W1*x[(idx-2)*N+i]));
                }
                return;
            }
            void GPU_Exponent( float * x, size_t idx, size_t N, float W0, float W1)
            {
                Exponent<<< DIM_GRID, DIM_BLOCK >>>(x, idx, N, W0, W1);
            }
            /// Evaluates the node and updates the stack states. 
            /* void NodeExponent::evaluate(const MatrixXf& X, const VectorXf& y, vector<ArrayXf>& stack_f, */ 
            ...
            /*     Exponent<<< 32*numSMs, 128 >>>(dev_x1, dev_x2, dev_res, N); */
