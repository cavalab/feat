   	...
   	namespace Pop{
   	    namespace Op{	   		
            ...
            {                    
                ...
                {
                    x[(idx-1)*N+i] = limited(exp ( - pow ( W0 - x[(idx-1)*N+i], 2 ) ) ) ;
                }
                return;
            }
            ...
            /* void NodeGaussian::evaluate(const MatrixXf& X, const VectorXf& y, vector<ArrayXf>& stack_f, */ 
            ...
            /*     Gaussian<<< 32*numSMs, 128 >>>(dev_x1, dev_res, N); */
