...
namespace FT{
   	namespace Pop{
   	    namespace Op{	
            ...
            __global__ void Cube( float * x, size_t idx, size_t N, float W0)
            {  
                ...
                {
                    //printf("From cube %f, %f \n", (W0*x[(idx-1)*N+i]), pow(W0*x[(idx-1)*N+i] , 3 ));
                    x[(idx-1)*N+i] = pow(W0*x[(idx-1)*N+i] , 3 );
            ...
            {
                Cube<<< DIM_GRID, DIM_BLOCK >>>(x, idx, N, W0);
            }
            /// Evaluates the node and updates the stack states. 
            /* void NodeCube::evaluate(const MatrixXf& X, const VectorXf& y, vector<ArrayXf>& stack_f, */ 
