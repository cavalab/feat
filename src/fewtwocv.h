...
namespace FT{
    ...
    class FewtwoCV
    {
        ...
        FewtwoCV(vector<string> mlStr, int popRange = 100, int genRange = 100, int dimRange = 10, int depRange = 3);    
    ...
    FewtwoCV::FewtwoCV(vector<string> mlStr, int popRange, int genRange, int dimRange, int depRange)
    {
    	ml = mlStr;
    ...
    void FewtwoCV::create_folds(MatrixXd &x, VectorXd &y)
    {
    ...
    vector<char> FewtwoCV::fold_dtypes(MatrixXd &X)
    {
    	int i, j;
	    bool isBinary;
    ...
    void FewtwoCV::validate_data(MatrixXd x, VectorXd &y)
    {
