...
template<class ST> CMyDenseFeatures<ST>::CMyDenseFeatures(int32_t size) : CDotFeatures(size)
{
	init();
}

template<class ST> CMyDenseFeatures<ST>::CMyDenseFeatures(const CMyDenseFeatures & orig) :
		CDotFeatures(orig)
{
	init();
...
template<class ST> CMyDenseFeatures<ST>::CMyDenseFeatures(SGMatrix<ST> matrix) :
		CDotFeatures()
{
	init();
...
template<class ST> CMyDenseFeatures<ST>::CMyDenseFeatures(ST* src, int32_t num_feat, int32_t num_vec) :
		CDotFeatures()
{
	init();
	set_feature_matrix(SGMatrix<ST>(src, num_feat, num_vec));
}
template<class ST> CMyDenseFeatures<ST>::CMyDenseFeatures(CFile* loader) :
		CDotFeatures()
{
	init();
	load(loader);
...
template<class ST> CFeatures* CMyDenseFeatures<ST>::duplicate() const
{
	return new CMyDenseFeatures<ST>(*this);
}

template<class ST> CMyDenseFeatures<ST>::~CMyDenseFeatures()
{
	free_features();
}

template<class ST> void CMyDenseFeatures<ST>::free_features()
{
	free_feature_matrix();
	SG_UNREF(feature_cache);
}

template<class ST> void CMyDenseFeatures<ST>::free_feature_matrix()
{
...
template<class ST> ST* CMyDenseFeatures<ST>::get_feature_vector(int32_t num, int32_t& len, bool& dofree)
{
...
template<class ST> void CMyDenseFeatures<ST>::set_feature_vector(SGVector<ST> vector, int32_t num)
{
	/* index conversion for subset, only for array access */
	int32_t real_num=m_subset_stack->subset_idx_conversion(num);
...
template<class ST> SGVector<ST> CMyDenseFeatures<ST>::get_feature_vector(int32_t num)
{
	/* index conversion for subset, only for array access */
	int32_t real_num=m_subset_stack->subset_idx_conversion(num);
...
template<class ST> void CMyDenseFeatures<ST>::free_feature_vector(ST* feat_vec, int32_t num, bool dofree)
{
	if (feature_cache)
...
template<class ST> void CMyDenseFeatures<ST>::free_feature_vector(SGVector<ST> vec, int32_t num)
{
	free_feature_vector(vec.vector, num, false);
	vec=SGVector<ST>();
}

template<class ST> void CMyDenseFeatures<ST>::vector_subset(int32_t* idx, int32_t idx_len)
{
...
template<class ST> void CMyDenseFeatures<ST>::feature_subset(int32_t* idx, int32_t idx_len)
{
	if (m_subset_stack->has_subsets())
...
template <class ST>
SGMatrix<ST> CMyDenseFeatures<ST>::get_feature_matrix()
{
	if (!m_subset_stack->has_subsets())
...
template <class ST>
void CMyDenseFeatures<ST>::copy_feature_matrix(SGMatrix<ST> target, index_t column_offset) const
{
	REQUIRE(column_offset>=0, "Column offset (%d) cannot be negative!\n", column_offset);
	//cout << "target is \n"; target.display_matrix();
...
template<class ST> SGMatrix<ST> CMyDenseFeatures<ST>::steal_feature_matrix()
{
...
template<class ST> void CMyDenseFeatures<ST>::set_feature_matrix(SGMatrix<ST> matrix)
{
	m_subset_stack->remove_all_subsets();
	free_feature_matrix();
...
template<class ST> ST* CMyDenseFeatures<ST>::get_feature_matrix(int32_t &num_feat, int32_t &num_vec)
{
...
template<class ST> CMyDenseFeatures<ST>* CMyDenseFeatures<ST>::get_transposed()
{
	int32_t num_feat;
	int32_t num_vec;
	ST* fm = get_transposed(num_feat, num_vec);

	return new CMyDenseFeatures<ST>(fm, num_feat, num_vec);
}

template<class ST> ST* CMyDenseFeatures<ST>::get_transposed(int32_t &num_feat, int32_t &num_vec)
{
...
template<class ST> void CMyDenseFeatures<ST>::obtain_from_dot(CDotFeatures* df)
{
	m_subset_stack->remove_all_subsets();
...
template<class ST> bool CMyDenseFeatures<ST>::apply_preprocessor(bool force_preprocessing)
{
	if (m_subset_stack->has_subsets())
...
template<class ST> int32_t CMyDenseFeatures<ST>::get_num_vectors() const
{
	return m_subset_stack->has_subsets() ? m_subset_stack->get_size() : num_vectors;
}

template<class ST> int32_t CMyDenseFeatures<ST>::get_num_features() const { return num_features; }

template<class ST> void CMyDenseFeatures<ST>::set_num_features(int32_t num)
{
	num_features = num;
	initialize_cache();
}

template<class ST> void CMyDenseFeatures<ST>::set_num_vectors(int32_t num)
{
	if (m_subset_stack->has_subsets())
...
template<class ST> void CMyDenseFeatures<ST>::initialize_cache()
{
	if (m_subset_stack->has_subsets())
...
template<class ST> EFeatureClass CMyDenseFeatures<ST>::get_feature_class() const  { return C_DENSE; }

template<class ST> bool CMyDenseFeatures<ST>::reshape(int32_t p_num_features, int32_t p_num_vectors)
{
	if (m_subset_stack->has_subsets())
...
template<class ST> int32_t CMyDenseFeatures<ST>::get_dim_feature_space() const { return num_features; }

template<class ST> float64_t CMyDenseFeatures<ST>::dot(int32_t vec_idx1, CDotFeatures* df,
		int32_t vec_idx2)
{
	ASSERT(df)
	ASSERT(df->get_feature_type() == get_feature_type())
	ASSERT(df->get_feature_class() == get_feature_class())
	CMyDenseFeatures<ST>* sf = (CMyDenseFeatures<ST>*) df;
...
template<class ST> void CMyDenseFeatures<ST>::add_to_dense_vec(float64_t alpha, int32_t vec_idx1,
...
template<>
void CMyDenseFeatures<float64_t>::add_to_dense_vec(float64_t alpha, int32_t vec_idx1,
		float64_t* vec2, int32_t vec2_len, bool abs_val)
{
	ASSERT(vec2_len == num_features)
...
template<class ST> int32_t CMyDenseFeatures<ST>::get_nnz_features_for_vector(int32_t num)
{
	return num_features;
}

template<class ST> void* CMyDenseFeatures<ST>::get_feature_iterator(int32_t vector_index)
{
...
template<class ST> bool CMyDenseFeatures<ST>::get_next_feature(int32_t& index, float64_t& value,
		void* iterator)
{
...
template<class ST> void CMyDenseFeatures<ST>::free_feature_iterator(void* iterator)
{
	if (!iterator)
		return;
...
template<class ST> CFeatures* CMyDenseFeatures<ST>::copy_subset(SGVector<index_t> indices)
{
	...
	CFeatures* result=new CMyDenseFeatures(feature_matrix_copy);
	SG_REF(result);
	return result;
}

template<class ST>
CFeatures* CMyDenseFeatures<ST>::copy_dimension_subset(SGVector<index_t> dims)
{
	SG_DEBUG("Entering!\n");
...
template<class ST>
CFeatures* CMyDenseFeatures<ST>::shallow_subset_copy()
{
	...
	shallow_copy_features=new CMyDenseFeatures<ST>(shallow_copy_matrix);
...
template<class ST> ST* CMyDenseFeatures<ST>::compute_feature_vector(int32_t num, int32_t& len,
		ST* target)
{
	SG_NOTIMPLEMENTED
	len = 0;
...
template<class ST> void CMyDenseFeatures<ST>::init()
{
	num_vectors = 0;
	num_features = 0;
...
template<> EFeatureType CMyDenseFeatures<sg_type>::get_feature_type() const \
{																			\
	return f_type;															\
}
...
template<> float64_t CMyDenseFeatures<bool>::dense_dot(int32_t vec_idx1,
...
template<> float64_t CMyDenseFeatures<char>::dense_dot(int32_t vec_idx1,
		const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<int8_t>::dense_dot(int32_t vec_idx1,
		const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<uint8_t>::dense_dot(
...
template<> float64_t CMyDenseFeatures<int16_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<uint16_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<int32_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<uint32_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<int64_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<uint64_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<float32_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<float64_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<> float64_t CMyDenseFeatures<floatmax_t>::dense_dot(
		int32_t vec_idx1, const float64_t* vec2, int32_t vec2_len)
{
	ASSERT(vec2_len == num_features)
...
template<class ST> bool CMyDenseFeatures<ST>::is_equal(CMyDenseFeatures* rhs)
{
...
template <class ST>
CFeatures* CMyDenseFeatures<ST>::create_merged_copy(CList* others)
{
	SG_DEBUG("Entering.\n");
	...
	{
		auto casted=dynamic_cast<CMyDenseFeatures<ST>*>(current);
	...
	while (current!=nullptr)
	{
		auto casted=static_cast<CMyDenseFeatures<ST>*>(current);
	...
	auto result=new CMyDenseFeatures<ST>(data);
...
template <class ST>
CFeatures* CMyDenseFeatures<ST>::create_merged_copy(CFeatures* other)
{
...
template<class ST>
void CMyDenseFeatures<ST>::load(CFile* loader)
{
	SGMatrix<ST> matrix;
	matrix.load(loader);
	set_feature_matrix(matrix);
...
template<class ST>
void CMyDenseFeatures<ST>::save(CFile* writer)
{
	feature_matrix.save(writer);
}

template< class ST > CMyDenseFeatures< ST >* CMyDenseFeatures< ST >::obtain_from_generic(CFeatures* const base_features)
{
	REQUIRE(base_features->get_feature_class() == C_DENSE,
			"base_features must be of dynamic type CMyDenseFeatures\n")

	return (CMyDenseFeatures< ST >*) base_features;
}

template class CMyDenseFeatures<bool>;
template class CMyDenseFeatures<char>;
template class CMyDenseFeatures<int8_t>;
template class CMyDenseFeatures<uint8_t>;
template class CMyDenseFeatures<int16_t>;
template class CMyDenseFeatures<uint16_t>;
template class CMyDenseFeatures<int32_t>;
template class CMyDenseFeatures<uint32_t>;
template class CMyDenseFeatures<int64_t>;
template class CMyDenseFeatures<uint64_t>;
template class CMyDenseFeatures<float32_t>;
template class CMyDenseFeatures<float64_t>;
template class CMyDenseFeatures<floatmax_t>;
}
