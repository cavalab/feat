...
def runClassifier(iter):
    customPrint(  '[' +str(iter) + ']: Running MLP Classifier')
    ...
    clf =  GridSearchCV( MLPClassifier(solver='lbfgs',max_iter=200) ,cv = 5, param_grid= {"alpha" : alphas, "epsilon" : epsilons } )
    customPrint( '[' +str(iter) + ']: Fitting the MLP')
...
def startParallelProcessing():
    ...
    print ( results )
    fUtils.writeToCSV( resultsPath + "/" +  dataset + ".csv",results,"mlp",n)
