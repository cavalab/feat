...
def runNaiveBayes(iter):
    customPrint(  '[' +str(iter) + ']: Running Naive Bayes')
...
def startParallelProcessing():
    ...
    p=Pool(n)
    results = p.map( runNaiveBayes, range(n)  )
    customPrint ('Writing results to csv file...')
    print ( results )
    fUtils.writeToCSV( resultsPath + "/" +  dataset + ".csv",results,"nb",n)
