...
def runClassifier(iter):
    ...
    fb = [0.2, 0.5, 0.8]
    ml = [ "LR".encode('utf-8'),"SVM".encode('utf-8'),"CART".encode('utf-8') ]
    cross_rate = [0.2, 0.8]
    
    try:
        clf = GridSearchCV(ft.Feat(max_depth=5,max_dim=min(50,2*features.shape[1]),verbosity=v,shuffle=True,classification=True),param_grid= {"fb" : fb, "ml" : ml, "cross_rate" : cross_rate },cv=5)
        customPrint( '[' +str(iter) + ']: Fitting Feat')
...
def startParallelProcessing():
    ...
    features_set = set(label) 
    #if ( len(features_set) > 2):
    #    print("Exiting Feat as it is  a multiclass Problem")
    #     quit()  
    ...
    print ( results )
    #fUtils.writeToCSV( resultsPath + "/" +  dataset + ".csv",results,"feat",n)
