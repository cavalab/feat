...
def runLinearRegression(iter):
    customPrint(  '[' +str(iter) + ']: Running Linear Tree Regressor')
    ...
    lr = lr.fit(features_train , labels_train)

    customPrint(  '[' +str(iter) + ']: Predicting the labels')
    labels_predicted_lr = lr.predict( features_test )
    customPrint  ('[' +str(iter) + '] MSE: ' + str(mse ( labels_test, labels_predicted_lr )))
    return mse ( labels_test, labels_predicted_lr )
...
def startParallelProcessing():
    ...
    p=Pool(n)
    results = p.map( runLinearRegression, range(n)  )
    customPrint ('Writing results to csv file...')
    fUtils.writeToCSV( resultsPath + "/" +  dataset + ".csv",results,"linreg",n)
