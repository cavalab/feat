...
def runLogisticRegression(iter):
    customPrint(  '[' +str(iter) + ']: Running Logistic Regression')
    ...
    clf =  GridSearchCV(LogisticRegression(max_iter=1000),cv = 5, param_grid= {"C" : C} ,n_jobs=1)
    customPrint( '[' +str(iter) + ']: Fitting the Logistic Regression ')
...
def startParallelProcessing():
    ...
    p=Pool(n)
    results = p.map( runLogisticRegression, range(n)  )
    customPrint ('Writing results to csv file...')
    print ( results )
    fUtils.writeToCSV( resultsPath + "/" +  dataset + ".csv",results,"logreg",n)
