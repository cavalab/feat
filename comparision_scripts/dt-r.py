...
def runDecisionTreeRegressor(iter):
    customPrint(  '[' +str(iter) + ']: Running Decision Regressor')
    ...
    labels_predicted_dt = dt.predict( features_test )
    customPrint  ('[' +str(iter) + '] MSE: ' + str(mse ( labels_test, labels_predicted_dt )))
    return mse ( labels_test, labels_predicted_dt )
...
def startParallelProcessing():
    ...
    p=Pool(n)
    results = p.map( runDecisionTreeRegressor, range(n)  )
